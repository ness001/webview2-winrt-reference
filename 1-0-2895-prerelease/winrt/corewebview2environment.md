...
- Microsoft.Web.WebView2.Core.CoreWebView2Environment.add_RestartRequested
...
- Microsoft.Web.WebView2.Core.CoreWebView2Environment.remove_RestartRequested
...
- Microsoft.Web.WebView2.Core.CoreWebView2Environment.RestartRequested
---
...
[RestartRequested](#restartrequested) | Event is raised when there is a need to restart WebView2 process in order to apply certain beneifical updates.
...
Event is raised when there is a need to restart WebView2 process in order to apply certain beneifical updates.
This API gives developers the awareness of these necessary WebView2 restarts, allowing developers to resolve issues faster than waiting for end users to restart the app. Developer might want to give end users the ability to save their state before restarting.
For apps with multiple processes that host WebView2s that share the same user data folder you need to make sure all WebView2 instances are closed and the associated WebView2 Runtime browser process exits. See [CoreWebView2Environment.BrowserProcessExited](corewebview2environment.md#browserprocessexited) for more details.
